<script lang="ts">
  ...
  async function createItem() {
    ...
    await api.createMenuItem(token, formData);
    name = "";
    imageFile = null;
    await load();
  }
...
<main class="mx-auto max-w-4xl space-y-4 px-4 py-6">
  ...
  >
    ...
    />
    <button
      class="rounded-xl bg-teal-700 px-4 py-2 text-white md:col-span-2"
  ...
  <ul class="space-y-2">
    {#each items as item}
      ...
      >
        ...
        <button class="text-sm text-red-700" onclick={() => removeItem(item.id)}
          >Delete</button
        >
      </li>
    {/each}
